...
class PetStats:
    ...
    def clamp(self, value: float, min_v=0.0, max_v=100.0):
        return max(min_v, min(max_v, value))

    def add(self, stat: str, amount: float):
...
class Pet:
    ...
    def feed(self):
        if self.is_alive and self.life_stage!= "EGG":
            self.stats.add("hunger", -20.0)
            self.state = "EATING"

    def play(self):
        if self.is_alive and self.life_stage!= "EGG":
            self.stats.add("happiness", 20.0)
            self.stats.add("energy", -10.0)
            self.stats.add("hunger", 5.0)  # small fullness decrease
    ...
    def load(self):
        if os.path.exists(SAVE_FILE):
            with open(SAVE_FILE, 'r') as f:
                data = json.load(f)
                self.stats.hunger = data.get("hunger", 50.0)
                self.stats.happiness = data.get("happiness", 100.0)
                self.stats.energy = data.get("energy", 100.0)
                self.stats.health = data.get("health", 100.0)
...
class GameEngine:
    ...
    def draw_pet_face(self, x, y, size):
        ...
        elif self.pet.stats.health < 50 or self.pet.stats.energy < 20:
        ...
        while running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.btn_feed.collidepoint(event.pos) and self.pet.stats.hunger > 0:
                        # Only boost energy if fullness is <= 80%
                        if (100 - self.pet.stats.hunger) <= 80:
                            self.pet.stats.add("energy", 5)
                        self.pet.feed()
                    elif self.btn_play.collidepoint(event.pos) and self.pet.stats.energy < 100: self.pet.play()
                    elif self.btn_sleep.collidepoint(event.pos):
                        if self.pet.stats.energy < 100:
                            self.pet.stats.add("energy", 30)
                            if self.pet.stats.energy <= 90:
                                self.pet.stats.add("hunger", 5.0)  # small fullness decrease
            ...
            if self.pet.life_stage == "EGG":
                ...
                if self.pet.stats.hunger > 80 and self.pet.is_alive:
                ...
                if self.pet.stats.energy < 20 and self.pet.is_alive:
                ...
                if self.pet.stats.happiness < 30 and self.pet.is_alive:
